...
function DrumKitForm({ onDrumKitAdded, api }) {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const res = await api.post('/drums', {
                brand,
                model,
                price: parseFloat(price)
            });
            onDrumKitAdded(res.data);
            setBrand('');
            setModel('');
            setPrice('');
            alert("DrumKit Saved!");
    ...
    return (
        <form onSubmit={handleSubmit} className="form-style">
            ...
            <button type="submit">Save DrumKit</button>
        </form>
    );
}
